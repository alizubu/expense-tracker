...
export function ProfileCard({ profiles, netBalance, onAdd }: ProfileListCardProps) {
  ...
  return (
    <Card className="flex flex-col w-full h-auto lg:h-full p-6 rounded-2xl shadow-sm border-white/[0.04] bg-surface-1 transition-shadow hover:shadow-md">
      ...
      <div className="flex flex-col lg:flex-1 lg:overflow-y-auto hide-scrollbar lg:min-h-0 space-y-3">
        {profiles.length === 0 ? (
          ...
          profiles.map((profile, i) => {
            ...
            return (
              <motion.div 
                ...
                className="group flex flex-col justify-center p-4 rounded-xl hover:bg-surface-2 cursor-pointer transition-colors relative border border-white/[0.02] bg-surface-1/50 flex-shrink-0"
              >
                <div className="flex items-center gap-4">
                  <div className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl shadow-sm ${colors.bg}`}>
                    <Icon size={20} className={colors.icon} />
                  </div>

                  <div className="flex flex-col flex-1 min-w-0">
                    <span className="text-sm font-semibold text-foreground truncate group-hover:text-primary transition-colors">{profile.name}</span>
                    <span className="text-xs text-muted-foreground truncate capitalize">{profile.type.toLowerCase()}</span>
                  </div>

                  <div className="flex flex-col items-end flex-shrink-0 text-right">
                    <span className="text-sm font-semibold text-foreground tabular-money tracking-tight">
                      {symbol}{profile.balance.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                    </span>
                    <span className={`hidden sm:flex items-center justify-center text-[10px] font-semibold mt-1 px-2 py-0.5 rounded-full ${colors.bg} ${colors.icon}`}>
